<x-app-layout>
    ...
    <div class="max-w-xl mx-auto">
        @foreach (App\Models\ReservationModel::where('approver_id', auth()->user()->id_users)->take(1)->get() as $reservation)
                    ...
                    <div class="mt-4">
                        ...
                        <select id="id_reservations" name="id_reservations"
                            ...
                            @foreach (App\Models\ReservationModel::where('approver_id', auth()->user()->id_users)->get() as $reservation)
                                <option value="{{ $reservation->id_reservations }}">{{ $reservation->vehicle->model }} - {{ $reservation->id_reservations }}</option>
                            @endforeach
                        </select>
